...
class Installer(object):
    ...
    def install_requirements(self):
        ...
        if distro.id() in ['ubuntu', 'linuxmint']:
            ...
            command = 'sudo apt-get install -y libmodule-install-perl apache2 bcftools build-essential cpanminus curl git libbz2-dev libcurl4-openssl-dev liblocal-lib-perl liblzma-dev default-libmysqlclient-dev libpng-dev libpq-dev libssl-dev manpages openssl perl perl-base pkg-config python3-dev python3-pip python3-setuptools sed tabix unzip vcftools vim wget zlib1g-dev apache2 build-essential cpanminus curl git libpng-dev libssl-dev locales manpages mariadb-client openssl perl perl-base unzip vim wget libgd-dev libxml-libxml-perl libgd-dev vcftools'  # lamp-server^
            sts = call(command, shell=True)
