...
class Vep(object):
    def __init__(self, args=None):
        ...
        if self.args.build == 'hg38':
            self.build = 'GRCh38'
        else:
            self.build = 'GRCh37'

        # print('ARGS',args)
        self.vcffile = 'sanity_check/sorted.vcf'
    ...
    def run(self):
        tstart = datetime.now()
        print(tstart, 'Starting vep annotation: ', self.args.build, self.vcffile)
    ...
    def annotate(self):
        ...
        if not os.path.exists(settings.vep_cache_dir):
            ...
            1>vep/vep.log \
            --fork %s \
            ''' % (settings.vep_dir, self.vcffile, self.build, settings.vep_cores)
        else:
